...
function m(e){return e&&e.__esModule?e:{default:e}}function p(e,t,n){return t in e?Object.defineProperty(e,t,{value:n,enumerable:!0,configurable:!0,writable:!0}):e[t]=n,e}var v,b={onMessageCB:function(){return v||(v=this.onMessage.bind(this))},onVisibilityChangeCB:function(){var e
...
break
case"frame_state_change":var k=document.getElementById(s.default.frameDivId),L=p&&p.data,O=L&&L.frameState,S=a.default.getSettings()&&a.default.getSettings().enabledFeatures
o.default.publish("frame:statechange",p),"initialized"===O&&(a.default.isJWTEnabled()&&a.default.isJWTStrictMode()&&k.classList.add("fc_dn"),this.loadWidget(),S&&-1!==S.indexOf("COBROWSING")&&g.default.init({host:a.default.getHost(),locale:d.default.getProperties().locale})),"loaded"===O&&a.default.loaded(!0),"authenticated"!==O&&"not_authenticated"!==O||a.default.isJWTEnabled()&&a.default.isJWTStrictMode()&&(p.success?k.classList.remove("fc_dn"):304!==p.status&&this.unloadWidget())
break
...
else if(p.success){var M=P&&P.externalId,T=P&&P.restoreId,A=d.default.getExternalId()
A?A===M&&(d.default.setRestoreId(T),this.onUserCreate(p)):this.onUserCreate(p)}else this.onUserCreate(p)
break
...
break
case"read_receipts_update":o.default.publish("readReceipts:update",p)
break
case"csat_updated":o.default.publish("csat:updated",p)
break
case"csat_received":o.default.publish("csat:received",p)
break
case"anchor_link_clicked":o.default.publish("anchorLink:clicked",p)
break
case"faq_clicked":o.default.publish("faq:clicked",p)
break
case"download_file":o.default.publish("download:file",p)
break
...
break
case"set_storage_item":if(i.default.storageAvailable){var x=p.data
x&&x.key&&x.value&&localStorage.setItem(x.key,JSON.stringify(x.value))}break
...
break
case"set_lsds_item":if(i.default.storageAvailable){var j=p.data,B=j&&j.key
if(B){var D=j&&j.value
localStorage.setItem(B,i.default.stringifyJSON(D))}}break
case"get_lsds_item":if(i.default.storageAvailable){var F=p.data,W=F&&F.key
W&&localStorage.getItem(W)}break
case"remove_lsds_item":if(i.default.storageAvailable){var q=p.data,V=q&&q.key
V&&localStorage.removeItem(V)}break
case"requestForAudioCall":window.fc_cobrowse&&window.fc_cobrowse.requestForAudioCall(e)
break
case"rts_connected":o.default.publish("rts:connected",p.data)
break
case"rts_disconnected":o.default.publish("rts:disconnected",p.data)
break
